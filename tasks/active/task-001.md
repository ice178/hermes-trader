...
- The sent-signal JSON file is mutable state and must live outside the deployed code on the server.

## Investigation and decision

Use a minimal `systemd` oneshot service with a timer that preserves the previous 15-minute schedule. Let `systemd` load a root-controlled environment file, avoiding a new dotenv dependency. Run as a dedicated unprivileged user, keep mutable state in `/var/lib/hermes-trading`, and use journald for logs.
...
3. Make the sent-signal state path configurable through environment.
...
- [x] Decide how the sent-signal state file will be preserved across deployments.
...
- [x] The application can receive all required runtime configuration without a new dependency.
...
- [x] Persistent runtime state and file permissions are documented.
...
Implementation is ready for review. Confirm that the target host uses `systemd`, revoke the historical token, create the protected server environment file, migrate `src/signals_bot_sent.json`, then follow `docs/server-deployment.md`. Focused tests: `3 passed`; full suite: `70 passed`.
